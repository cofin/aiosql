...
async def rconn(pg_dsn):
    conn = await asyncmy.connect(pg_dsn)
    yield conn
    await conn.close()
...
async def test_with_pool(pg_dsn, queries, pg_db):
    async with asyncmy.create_pool(pg_dsn) as pool:
...
def test_many_replacements(pg_dsn, queries):
    ...
    """
    actual = aiosql.from_str(sql, "asyncmy").test.sql
...
def test_maybe_order_params():
    a = aiosql.adapters.asyncmy.asyncmyAdapter()
    try:
